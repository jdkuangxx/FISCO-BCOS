    ...
    const LimitTuple& limit, uint32_t version) const
{
    ...
    #define PUSH_COND(_idx, _value, OP)                                                     \
        if(_idx == 0)                                                                       \
            keyCondition->OP(_value);                                                       \
        else                                                                                \
            valueCondition->OP(_idx, _value)  
    ...
    for (const auto& condition : conditions.cond_v320)
    {
        ...
        {
        case 0:
            PUSH_COND(field_idx, value, EQ);
            break;
        case 1:
            PUSH_COND(field_idx, value, NE);
            break;
        case 2:
            PUSH_COND(field_idx, value, GT);
            break;
        case 3:
            PUSH_COND(field_idx, value, GE);
            break;
        case 4:
            PUSH_COND(field_idx, value, LT);
            break;
        case 5:
            PUSH_COND(field_idx, value, LE);
            break;
        default:
            BOOST_THROW_EXCEPTION(
                PrecompiledError(std::to_string(cmp) + " ConditionOP not exist!"));
...

{
    ...
    } while (singleCountByKey >= USER_TABLE_MAX_LIMIT_COUNT);
